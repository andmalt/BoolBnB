...
import { House, MAIN_URL, Photos } from '../services/interfaces';
...
const HouseSmallCard = (props: HouseCard) => {
    ...
    return (
        <div className="flex flex-row flex-wrap space-x-6 dark:bg-[#0a121e] bg-slate-200 shadow-xl rounded-2xl">
                            ...
                            photos.length != 0 ?
                                <div className="duration-700 ease-in-out">
                                    <img src={photo?.image_url.includes("https://") ||
                                        photo?.image_url.includes("http://") ? photo?.image_url : `${MAIN_URL}storage/apartments/images/${photo?.image_url}`} className="block absolute top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2" alt={`image ${photo.id}`} />
                                </div>
                                :
                                <div className="duration-700 ease-in-out">
            ...
            <div onClick={() => getHome(id)} className="sm:w-7/12 pl-0 p-5 text-[#9ca3af] overflow-auto cursor-pointer">
                <div className="space-y-2">
                    <div className="space-y-4">
                        <h4 className="text-2xl font-semibold dark:text-white text-black text-left">{title}</h4>
